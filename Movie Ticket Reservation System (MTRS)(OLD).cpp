...
char name[25], icno[25], email[40], phone_number[25],acc_num[25],password[25],month[25], year[25];
...
class Member : public Customer {
    ...
    public:
         time_t t = time(NULL);  //get current year & month upon renew member regis
...
void add_movie()
{
    ...
    movie << name << '|'  << day << '|' << time[0] << '|' << time[1] << '|' << time[2];
    cout << endl<<endl;
...
void delete_movie(){
    ...
    cout << "\n Enter the serial number of the movie that you want to delete [1,2,3...] : ";
...
update_movie(){
    ...
    while(i++ , i <= lines)
    {
        ...
        if (i == selection_movie)
        {
            ...
            cin >> time[2];
            time_checking(time, x);
            temp << name << '|' << day << '|' << time[0] << '|' << time[1] << '|' << time[2] << '\n';
        }
...
void movie_booking()
{
    ...
    while (seat_no < 1){
        cout << "Error: Invalid seat numbers\n";
...
void ticket_cancel(){
    ...
    fstream file, booking, temp;
    int lines = 0;
    string search_receipt;
    char ticket_num[25], name[25], phone[25], email[25], seat_type[25], movie[25], showtime[25], seat_booked[60];
    bool isExist = 1, isFound = 1;
    char response;
    ...
    do{
        ...
        while(!file.eof())
        {
            file.getline(ticket_num,25,'|');
            file.getline(name,25,'|');
            file.getline(phone,25,'|');
            file.getline(email,25,'|');
            file.getline(movie,25,'|');
            file.getline(showtime,25,'|');
            file.getline(seat_type,25,'|');
            file.getline(seat_booked,60);

            for(int i=0; i<search_receipt.size(); i++)
            {
                if(ticket_num[i] == search_receipt[i]){
                    isExist = 1;   // TRUE
                }
                else{
                    isExist = 0;   // FALSE
